...
import torch.lmdb.flags.EnvFlags.MDB_FIXEDMAP
import torch.lmdb.flags.EnvFlags.MDB_NOSYNC
...
import torch.lmdb.flags.EnvFlags

/** Test {@link MaskedFlag}. */
final class MaskedFlagTest {
  @Test def isSetOperates(): Unit = {
  ...
  @Test def masking(): Unit = {
    val nullFlags: Array[EnvFlags] = null
    assertThat(mask(nullFlags*), is(0))
    val emptyFlags = new Array[EnvFlags](0)
    assertThat(mask(emptyFlags*), is(0))
    val nullElementZero = Array[EnvFlags](null)
