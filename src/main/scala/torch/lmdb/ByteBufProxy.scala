...
final class ByteBufProxy(private val nettyAllocator: PooledByteBufAllocator)
 ...
 */
  ...
  override protected def allocate: ByteBuf = {
  ...
  override protected def getSignedComparator: Comparator[ByteBuf] = ByteBufProxy.comparator

  override protected def getUnsignedComparator: Comparator[ByteBuf] = ByteBufProxy.comparator

  override protected def deallocate(buff: ByteBuf): Unit = {
    buff.release
  }

  override protected def getBytes(buffer: ByteBuf): Array[Byte] = {
    val dest = new Array[Byte](buffer.capacity)
    buffer.getBytes(0, dest)
    dest
  }

  override protected def in(buffer: ByteBuf, ptr: Pointer, ptrAddr: Long): Unit = {
  ...
  override protected def in(buffer: ByteBuf, size: Int, ptr: Pointer, ptrAddr: Long): Unit = {
    UNSAFE.putLong(ptrAddr + STRUCT_FIELD_OFFSET_SIZE, size)
    UNSAFE.putLong(ptrAddr + STRUCT_FIELD_OFFSET_DATA, buffer.memoryAddress + buffer.readerIndex)
  }

  override protected def out(buffer: ByteBuf, ptr: Pointer, ptrAddr: Long): ByteBuf = {
