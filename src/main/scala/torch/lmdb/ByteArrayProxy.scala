...
object ByteArrayProxy {
  ...
  private val signedComparator = ByteArrayProxy.compareArraysSigned
  private val unsignedComparator = ByteArrayProxy.compareArrays
...
final class ByteArrayProxy private extends BufferProxy[Array[Byte]] {
  override protected def allocate = new Array[Byte](0)

  override protected def deallocate(buff: Array[Byte]): Unit = {

    // byte arrays cannot be allocated
  }

  override protected def getBytes(buffer: Array[Byte]): Array[Byte] = util.Arrays.copyOf(buffer, buffer.length)

  override protected def getSignedComparator: Comparator[Array[Byte]] = ByteArrayProxy.signedComparator

  override protected def getUnsignedComparator: Comparator[Array[Byte]] = ByteArrayProxy.unsignedComparator

  override protected def in(buffer: Array[Byte], ptr: Pointer, ptrAddr: Long): Unit = {
  ...
  override protected def in(buffer: Array[Byte], size: Int, ptr: Pointer, ptrAddr: Long): Unit = {

    // cannot reserve for byte arrays
  }

  override protected def out(buffer: Array[Byte], ptr: Pointer, ptrAddr: Long): Array[Byte] = {
