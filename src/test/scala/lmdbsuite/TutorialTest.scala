...
import torch.lmdb.enums.GetOp.MDB_SET
...
final class TutorialTest {
  ...
  def tutorial1(): Unit = {
    ...
    val path = new File("./tmp") // tmp.newFolder
    // We always need an Env. An Env owns a physical on-disk storage file. One
    // Env can store many different databases (ie sorted maps).
    val env = create.setMapSize(10_485_760).setMaxDbs(1).open(path)
    // We need a Dbi for each DB. A Dbi roughly equates to a sorted map. The
    // MDB_CREATE flag causes the DB to be created if it doesn't already exist.
    val db = env.openDbi(TutorialTest.DB_NAME, MDB_CREATE)
  ...
  def tutorial4(): Unit = {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          try{
            for (kv <- ci) {
              ...
              assertThat(kv.vals, notNullValue)
            }
          }
          finally
          {
        ...
        try {
          val ci = db.iterate(txn, KeyRange.allBackward)
          try{
            for (kv <- ci) {
              assertThat(kv.key, notNullValue)
              assertThat(kv.vals, notNullValue)
  ...
  def tutorial5(): Unit = {
    ...
    val db = env.openDbi(TutorialTest.DB_NAME, MDB_CREATE, MDB_DUPSORT)
    ...
    try {
      ...
      try {
        ...
        c.seek(MDB_FIRST)
        assertThat(UTF_8.decode(c.vals).toString, is("kkk"))
        c.seek(MDB_LAST)
        assertThat(UTF_8.decode(c.vals).toString, is("xxx"))
        c.seek(MDB_PREV)
        assertThat(UTF_8.decode(c.vals).toString, is("lll"))
        c.close()
        txn.commit()
      } finally if (txn != null) txn.close()
    }
  ...
  def tutorial6(): Unit = {
    ...
    val env = create(PROXY_OPTIMAL).setMapSize(10_485_760).setMaxDbs(Verifier.DBI_COUNT).open(tmp.newFolder)
  ...
  private def createSimpleEnv(path: File) = create.setMapSize(10_485_760).setMaxDbs(1).setMaxReaders(1).open(path)
}