...
import torch.lmdb.Dbi.KeyExistsException.MDB_KEYEXIST
import torch.lmdb.Dbi.KeyNotFoundException.MDB_NOTFOUND
import torch.lmdb.Env.SHOULD_CHECK
import torch.lmdb.KeyRange.all
...
import torch.lmdb.PutFlags.MDB_NODUPDATA
import torch.lmdb.PutFlags.MDB_NOOVERWRITE
import torch.lmdb.PutFlags.MDB_RESERVE
...
import torch.lmdb.Library.ComparatorCallback
...
final class Dbi[T] private[torch](private val env: Env[T], txn: Txn[T], name: Array[Byte], comparator: Comparator[T], nativeCb: Boolean, proxy: BufferProxy[T], flags: DbiFlags*) {
  if (SHOULD_CHECK) {
    requireNonNull(txn)
    txn.checkReady()
  }
  ...
  final private var name: Array[Byte] = null
 ...
 * Dbi}. Advanced users are expected to have specific reasons for using this method and will
   ...
   * @return true if the key/data pair was found, false otherwise
   */
  def delete(txn: Txn[T], key: T, vals: T): Boolean = {
    if (SHOULD_CHECK) {
      requireNonNull(txn)
      requireNonNull(key)
      txn.checkReady()
    ...
    ReferenceUtil.reachabilityFence0(vals)
    true
  }
  ...
  def get(txn: Txn[T], key: T): T = {
    ...
    checkRc(rc)
    ReferenceUtil.reachabilityFence0(key)
    txn.kv.valOut // marked as out in LMDB C docs
  }
 ...
 * Cursor#renew(org.lmdbjava.Txn)}. It can be discarded with {@link Cursor# close ( )}. A cursor in a
  ...
  def put(txn: Txn[T], key: T, vals: T, flags: PutFlags*): Boolean = {
    ...
    txn.kv.keyIn(key)
    txn.kv.valIn(vals)
    val mask = mask(true, flags)
    val rc = LIB.mdb_put(txn.pointer, ptr, txn.kv.pointerKey, txn.kv.pointerVal, mask)
    if (rc == MDB_KEYEXIST) {
      if (isSet(mask, MDB_NOOVERWRITE)) txn.kv.valOut // marked as in,out in LMDB C docs
      else if (!isSet(mask, MDB_NODUPDATA)) checkRc(rc)
      return false
    }
    checkRc(rc)
    ReferenceUtil.reachabilityFence0(key)
