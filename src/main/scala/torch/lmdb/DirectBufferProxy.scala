...
object DirectBufferProxy {
  private val signedComparator = (o1: DirectBuffer, o2: DirectBuffer) => {
    requireNonNull(o1)
    requireNonNull(o2)
    o1.compareTo(o2)
  }
  private val unsignedComparator = DirectBufferProxy.compareBuff
  /**
...
final class DirectBufferProxy private extends BufferProxy[DirectBuffer] {
  override protected def allocate: DirectBuffer = {
  ...
  override protected def getSignedComparator: Comparator[DirectBuffer] = DirectBufferProxy.signedComparator

  override protected def getUnsignedComparator: Comparator[DirectBuffer] = DirectBufferProxy.unsignedComparator

  override protected def deallocate(buff: DirectBuffer): Unit = {
    val q = DirectBufferProxy.BUFFERS.get
    q.offer(buff)
  }

  override protected def getBytes(buffer: DirectBuffer): Array[Byte] = {
    val dest = new Array[Byte](buffer.capacity)
    buffer.getBytes(0, dest, 0, buffer.capacity)
    dest
  }

  override protected def in(buffer: DirectBuffer, ptr: Pointer, ptrAddr: Long): Unit = {
  ...
  override protected def in(buffer: DirectBuffer, size: Int, ptr: Pointer, ptrAddr: Long): Unit = {
    val addr = buffer.addressOffset
    UNSAFE.putLong(ptrAddr + STRUCT_FIELD_OFFSET_DATA, addr)
    UNSAFE.putLong(ptrAddr + STRUCT_FIELD_OFFSET_SIZE, size)
  }

  override protected def out(buffer: DirectBuffer, ptr: Pointer, ptrAddr: Long): DirectBuffer = {
