...
import torch.lmdb.ByteBufferProxy.AbstractByteBufferProxy.findField
...
object ByteBufferProxy {
  ...
  private[torch] object AbstractByteBufferProxy {
    protected val FIELD_NAME_ADDRESS = "address"
    protected val FIELD_NAME_CAPACITY = "capacity"
    private val signedComparator = (o1: ByteBuffer, o2: ByteBuffer) => {
      requireNonNull(o1)
      requireNonNull(o2)
      o1.compareTo(o2)
    }
    private val unsignedComparator = AbstractByteBufferProxy.compareBuff
    /**
  ...
  abstract private[torch] class AbstractByteBufferProxy extends BufferProxy[ByteBuffer] {
    final protected def address(buffer: ByteBuffer): Long = {
      ...
      buffer.asInstanceOf[DirectBuffer].address + buffer.position
    }

    override final protected def allocate: ByteBuffer = {
    ...
    override protected def getSignedComparator: Comparator[ByteBuffer] = AbstractByteBufferProxy.signedComparator

    override protected def getUnsignedComparator: Comparator[ByteBuffer] = AbstractByteBufferProxy.unsignedComparator

    override final protected def deallocate(buff: ByteBuffer): Unit = {
      buff.order(BIG_ENDIAN)
      val queue = AbstractByteBufferProxy.BUFFERS.get
      queue.offer(buff)
    }

    override protected def getBytes(buffer: ByteBuffer): Array[Byte] = {
  ...
  final private class ReflectiveProxy extends ByteBufferProxy.AbstractByteBufferProxy {
    override protected def in(buffer: ByteBuffer, ptr: Pointer, ptrAddr: Long): Unit = {
      ptr.putAddress(STRUCT_FIELD_OFFSET_DATA, address(buffer))
      ptr.putLong(STRUCT_FIELD_OFFSET_SIZE, buffer.remaining)
    }

    override protected def in(buffer: ByteBuffer, size: Int, ptr: Pointer, ptrAddr: Long): Unit = {
      ptr.putLong(STRUCT_FIELD_OFFSET_SIZE, size)
      ptr.putAddress(STRUCT_FIELD_OFFSET_DATA, address(buffer))
    }

    override protected def out(buffer: ByteBuffer, ptr: Pointer, ptrAddr: Long): ByteBuffer = {
