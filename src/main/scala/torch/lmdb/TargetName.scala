...
object TargetName {
  ...
  private val EMBED = getProperty(LMDB_EMBEDDED_LIB_PROP)
  private val EXTERNAL = getProperty(LMDB_NATIVE_LIB_PROP)
  private val OS = getProperty("os.name")
  ...
  private[torch] def resolveFilename(external: String, embed: String, arch: String, os: String): String = {
    ...
    pkg + "/" + resolveArch(arch) + "-" + resolveOs(os) + "-" + resolveToolchain(os) + "." + resolveExtension(os)
  }
   ...
   */
  private def check(string: String, candidates: String*): Boolean = {
    if (string == null) return false
    val strLower = string.toLowerCase(ENGLISH)
    for (c <- candidates) {
      if (strLower.startsWith(c.toLowerCase(ENGLISH))) return true
    }
    false
  }
  ...
  private def resolveArch(arch: String): String = {
    ...
    else if (check(arch, "x86_64", "amd64")) return "x86_64"
    throw new UnsupportedOperationException(err("Unsupported os.arch"))
  }

  private def resolveOs(os: String): String = {
    if (check(os, "Linux")) return "linux"
    else if (check(os, "Mac OS")) return "macos"
    else if (check(os, "Windows")) return "windows"
    throw new UnsupportedOperationException(err("Unsupported os.name"))
  }

  private def resolveToolchain(os: String) = if (check(os, "Mac OS")) "none"
  else "gnu"

  try IS_EXTERNAL = isExternal(EXTERNAL)
  RESOLVED_FILENAME = resolveFilename(EXTERNAL, EMBED, ARCH, OS)
}

final class TargetName private {
}