...
import torch.lmdb.PutFlags.MDB_MULTIPLE
...
import torch.lmdb.SeekOp.MDB_FIRST
import torch.lmdb.SeekOp.MDB_LAST
import torch.lmdb.SeekOp.MDB_NEXT
import torch.lmdb.SeekOp.MDB_PREV
import jnr.ffi.Pointer
import jnr.ffi.byref.NativeLongByReference
import torch.lmdb.GetOp.GetOp
 ...
 * DbiFlags#MDB_DUPSORT}.
  ...
  def put(key: T, vals: T, op: PutFlags*): Boolean = {
    ...
    kv.keyIn(key)
    kv.valIn(vals)
    val mask = mask(true, op*)
    val rc = LIB.mdb_cursor_put(ptrCursor, kv.pointerKey, kv.pointerVal, mask)
    if (rc == MDB_KEYEXIST) {
      if (isSet(mask, MDB_NOOVERWRITE)) kv.valOut // marked as in,out in LMDB C docs
  ...
  def putMultiple(key: T, vals: T, elements: Int, op: PutFlags*): Unit = {
    ...
    val mask = mask(true, op)
    if (SHOULD_CHECK && !isSet(mask, MDB_MULTIPLE)) throw new IllegalArgumentException("Must set " + MDB_MULTIPLE + " flag")
    txn.kv.keyIn(key)
    val dataPtr = txn.kv.valInMulti(vals, elements)
    val rc = LIB.mdb_cursor_put(ptrCursor, txn.kv.pointerKey, dataPtr, mask)
    checkRc(rc)
    ReferenceUtil.reachabilityFence0(key)
    ReferenceUtil.reachabilityFence0(vals)
  }
