...
object Library {
   ...
   * LMDB system library will be extracted from the torch.lmdb JAR. If unspecified the LMDB system
   * library is extracted to the <code>java.io.tmpdir</code>. Ignored if the LMDB system library is
   * not being extracted from the torch.lmdb JAR (as would be the case if other system properties
  ...
  private def extract(name: String) = {
    ...
    try {
      ...
      file = createTempFile("torch.lmdb-native-library-", suffix, dir)
      file.deleteOnExit()
      ...
      try {
        ...
        try {
          ...
          while ({bytes = in.read(buffer); bytes != -1}) out.write(buffer, 0, bytes)
//          while ({-1 != (bytes = in.read(buffer))}) out.write(buffer, 0, bytes)
  ...
  final class MDB_envinfo private[torch](runtime: Runtime) extends Struct(runtime) {
    ...
    final var f2_me_last_pgno: Struct#size_t = new size_t
    final var f3_me_last_txnid: Struct#size_t = new size_t
    final var f4_me_maxreaders: Struct#u_int32_t = new u_int32_t
    final var f5_me_numreaders: Struct#u_int32_t = new u_int32_t
...
//    this.f5_me_numreaders = new Struct#u_int32_t
  ...
  final class MDB_stat private[torch](runtime: Runtime) extends Struct(runtime) {
    ...
    final var f2_ms_branch_pages: Struct#size_t = new size_t
    final var f3_ms_leaf_pages: Struct#size_t =  new size_t
    final var f4_ms_overflow_pages: Struct#size_t = new size_t
    final var f5_ms_entries: Struct#size_t = new size_t
