...
import torch.lmdb.EnvInfo
import torch.lmdb.db.Library.MDB_envinfo
...
import torch.lmdb.flags.{CopyFlags, DbiFlags, EnvFlags, TxnFlags}
...
object Env {
  ...
  val DISABLE_CHECKS_PROP = "torch.lmdb.disable.checks"
  /**
   * Indicates whether optional checks should be applied in torch.lmdb. Optional checks are only
 ...
 * ByteBufferProxy#PROXY_OPTIMAL}.
  ...
  final class Builder[T] private[torch](private val proxy: BufferProxy[T]) {
     ...
     * @return an environment ready for use
     */
    def open(path: File, flags: EnvFlags*): Env[T] = open(path, 0x1b4, flags*)
  ...
  @SerialVersionUID(1L)
  final class InvalidCopyDestination(message: String) extends LmdbException(message) {
  }
...
final class Env[T] private(private val proxy: BufferProxy[T], private val ptr: Pointer, private val readOnly: Boolean, private val noSubDir: Boolean) extends AutoCloseable { // cache max key size to avoid further JNI calls
   ...
   */
  def getDbiNames: List[Array[Byte]] = {
    ...
    try {
      ...
      try {
        if (!cursor.first) return List.empty //Collections.emptyList
